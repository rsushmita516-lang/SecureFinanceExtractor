...
import { Landmark, ShieldCheck, Mail, Lock, User, ArrowRight, Sparkles, AlertCircle } from "lucide-react";
...
export default function RegisterPage() {
  ...
  async function onSubmit(event: FormEvent) {
    ...
    if (!response.ok) {
      setError("Registration rejected. Email may already be associated with a workspace.");
      return;
    }
      ...
      <div className="w-full max-w-md relative z-10 space-y-6">
        ...
        <div className="flex flex-col items-center text-center space-y-2">
          ...
          <div>
            <h1 className="text-xl font-black text-slate-800 tracking-tight flex items-center justify-center gap-1.5">
              <span>Join Vessify Secure</span>
              <span className="rounded bg-emerald-50 px-1.5 py-0.5 text-[8px] font-bold uppercase text-emerald-800 border border-emerald-100">Free</span>
            </h1>
            <p className="text-xs text-slate-400 font-semibold mt-0.5">Initialize your isolated ledger container</p>
          </div>
        </div>
        ...
        <Card className="border-slate-200/60 shadow-xl bg-white/95 backdrop-blur-sm rounded-2xl overflow-hidden">
          ...
          <CardHeader className="space-y-1.5 pb-5">
            <CardTitle className="text-xl font-bold text-slate-800 text-center">Create Operator Account</CardTitle>
            <CardDescription className="text-slate-400 text-xs text-center font-medium">
              We will automatically construct and configure a private tenant database for you.
            </CardDescription>
          </CardHeader>
          <CardContent>
            <form className="space-y-4" onSubmit={onSubmit}>
              ...
              <div className="space-y-1.5">
                <Label htmlFor="name" className="text-xs font-bold text-slate-500 uppercase tracking-wider">Operator Name</Label>
              ...
              <div className="space-y-1.5">
                <Label htmlFor="password" className="text-xs font-bold text-slate-500 uppercase tracking-wider">Create Access Password</Label>
                <div className="relative">
              ...
              <Button type="submit" className="w-full h-10 bg-slate-900 hover:bg-slate-800 text-white font-bold text-xs rounded-xl flex items-center justify-center gap-1.5 transition cursor-pointer" disabled={loading}>
                {loading ? "Constructing Isolated Tenant..." : "Register & Provision"}
              ...
              <div className="pt-3 border-t border-slate-100 text-center">
                <p className="text-xs text-slate-400 font-medium">
                  Already registered?{" "}
                  <a href="/login" className="font-bold text-slate-800 hover:text-emerald-600 underline underline-offset-4 transition">
                    Sign in to session
                  </a>
                </p>
              </div>
